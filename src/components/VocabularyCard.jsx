...
const VocabularyCard = ({vocabulary = {}}) => {
    ...
    useEffect(() =>{
        if(difficulty === "easy"){
            setCardColor("bg-[#AEDDCB] text-black")
        } else if(difficulty === "medium"){
            setCardColor("bg-[#E7C4DB] text-black")
        } else if(difficulty === "difficult"){
            setCardColor("bg-[#F1B2B2] text-black")
        }
    },[])
    return (
        <div className={`border p-4 rounded-lg shadow-lg flex flex-col ${cardColor}`}>
            ...
            <Button variant="filled" className="bg-[#a38ffd] ">When to say</Button> 
        </div>
    );
};
